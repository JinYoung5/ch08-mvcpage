...
<div class="page-main">
	...
	<ul>
		...
		<li class="align-center">
			...
			<input type="button" value="배송지정보 수정" onclick="location.href='modifyForm.do?order_num=${order.order_num}'">
			</c:if>
			<c:if test="${order.status != 5}">
			<input type="button" value="배송상태수정" onclick="location.href='modifyStatusForm.do?order_num=${order.order_num}'">
			</c:if>
			<input type="button" value="구매목록" onclick="location.href='adminList.do'">
		</li>
	</ul>
	</div>
</div>
