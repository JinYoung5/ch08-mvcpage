		...
		<form action="adminList.do" method="get" id="search_form">
			<ul class="search">
				<li>
					<select name="keyfield" id="keyfield">
						<option value="1" <c:if test="${param.keyfield == 1}">selceted</c:if>>주문번호</option>
						<option value="2" <c:if test="${param.keyfield == 2}">selceted</c:if>>ID</option>
						<option value="3" <c:if test="${param.keyfield == 3}">selceted</c:if>>상품명</option>
					</select>
				</li>
				<li>
 		...
 		<c:if test="${count > 0}">
 			<table>
 				...
 				<tr>
 					<td><a href="adminDetail.do?order_num=${order.order_num}">${order.order_num}</a></td>
 					<td>${order.id}</td>
