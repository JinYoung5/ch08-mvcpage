...
public class CartDAO {
	...
	public int getTotalByMem_num(int mem_num)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT SUM(sub_total) FROM (SELECT mem_num, order_quantity * price sub_total FROM zcart JOIN zitem USING(item_num)) WHERE mem_num=?";
	...
	public List<CartVO> getListCart(int mem_num)throws Exception{
		...
		try {
			...
			conn = DBUtil.getConnection();
			//SQL문 작성
			sql = "SELECT * FROM zcart JOIN zitem USING(item_num) WHERE mem_num = ? ORDER BY item_num ASC";
			//PreparedStatement 객체 생성
			pstmt = conn.prepareStatement(sql);
			//?에 데이터 바인딩
			pstmt.setInt(1, mem_num);
			...
			while(rs.next()) {
				...
				//같은 상품(item_num이 같은 상품)의 총 구매 금액 구하기
